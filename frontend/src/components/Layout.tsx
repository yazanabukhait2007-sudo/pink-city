...
import { Phone, MapPin, Mail, Clock, ShieldCheck, Truck, RefreshCw, AlertCircle } from 'lucide-react';
import { isMockModeActive } from '../utils/api';
...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onNavigate }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50/50">
      ...
      {mockActive && (
        <div className="bg-amber-50 border-b border-amber-100 py-3 px-4 text-amber-900 text-sm">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3 text-right">
            <div className="flex items-center gap-3 justify-end flex-1">
              ...
              <div>
                <p className="font-extrabold text-xs">معاينة تفاعلية تجريبية (بيانات تجريبية نشطة)</p>
                <p className="text-[10px] text-amber-700 font-medium leading-relaxed mt-0.5">
                  قاعدة بيانات المعرض غير متصلة حالياً. قمنا بتشغيل وضع المحاكاة التلقائي ليتسنى لك تجربة كامل عمليات الشراء، تصفح المنتجات، السلة، التتبع، وإدارة الطلبات بلوحة الموظفين والمسؤولين بنجاح تام!
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-black bg-amber-200/60 text-amber-800 px-2 py-0.5 rounded-full uppercase shrink-0">وضع المحاكاة نشط</span>
            </div>
          </div>
        </div>
      )}
